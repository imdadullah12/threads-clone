...
const Threads = ({ refresh, setRefresh }) => {
  ...
  const getData = async () => {
    const request = await fetch("http://172.20.10.3/API/get/feeds.php", {
      method: "POST",
      headers: {
  ...
  const handleLike = async (id) => {
    const request = await fetch("http://172.20.10.3/API/likes/index.php", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const request = await fetch("http://172.20.10.3/API/posts/delete.php", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  return (
    <>
      ...
      {threads?.map((item, index) => {
        return (
          <div className="w-[45%] m-auto" key={index}>
